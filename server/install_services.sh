...
#               SERVICES="tts brain forward" (subset to install; add "tts2" for a second voice, "show" for the scheduled
#               two-unit news manzai - needs BOARD_A/BOARD_B URLs, runs 12:10 and 19:10 when the owner was seen recently)
set -e
...
BOARD_A="${BOARD_A:-http://$BOARD_IP}"; BOARD_B="${BOARD_B:-}"
...
for s in ${=SERVICES}; do
  ...
  sed -e "s|__REPO__|$REPO|g" -e "s|__PY__|$PY|g" -e "s|__PY_ML__|$PY_ML|g" -e "s|__BOARD_IP__|$BOARD_IP|g" \
      ...
      -e "s|__OLLAMA_SSH_HOST__|$OLLAMA_SSH_HOST|g" -e "s|__TTS_PITCH__|$TTS_PITCH|g" -e "s|__BOARD_A__|$BOARD_A|g" -e "s|__BOARD_B__|$BOARD_B|g" -e "s|<string>stackchan-$s</string>|<string>$label</string>|" -e "s|/tmp/stackchan-$s.log|/tmp/$label.log|g" \
